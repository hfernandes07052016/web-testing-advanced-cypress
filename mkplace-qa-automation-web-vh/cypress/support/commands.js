...
Cypress.Commands.add('api_removeCreditCard', (id) => {
  let accessToken = Cypress.env("customerAccessToken")
  let idCreditCard = id
  cy.request({
    method: 'DELETE',
    url: `${Cypress.env('baseUrlSandBox')}/payment/creditCard/`+ idCreditCard,
...
const getCartaoNumero = () => {
  return cy.request({
    ...
    return Cypress.$('#cartao_numero', response.body).text().trim();
  });
};

export default getCartaoNumero;
